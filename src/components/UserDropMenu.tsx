  ...
  return (
    <div>
      <button
        ...
        onClick={onClick}
      >
          <Image src="/user.svg" alt="user" width={30} height={30} className="object-cover rounded-full" />
      </button>
      {isOpen ? (
        <div className="absolute overflow-hidden right-16 border border-gray-200 shadow-lg px-3 py-2 rounded-lg">
          ...
          <Link href="/api/auth/signout" className="cursor-pointer block text-red-500 my-1">
            Logout
          </Link>
        </div>
      ) : undefined}
